        ...
        <div className="min-h-screen bg-slate-50 flex">
            ...
            <main className="flex-grow p-12">
                ...
                <AnimatePresence mode="wait">
                    {activeTab === 'overview' && <OverviewTab key="overview" data={dashboardData} onApprove={handleApproveApp} onReject={handleRejectApp} />}
                    {activeTab === 'authorization' && <AuthorizationTab key="auth" onRefresh={() => adminApi.getUsers()} />}
...
function OverviewTab({ data, onApprove, onReject }: OverviewTabProps) {
    if (!data) return null;
...
function AuthorizationTab({ onRefresh }: { onRefresh: () => Promise<any> }) {
...
function StatCard({ label, value, trend, trendType }: { label: string, value: string | number, trend: string, trendType: 'up' | 'down' | 'neutral' }) {
    return (
...
function AuthRow({ name, role, id, date }: { name: string, role: string, id: string, date: string }) {
    return (
...
function MenuEditCard({ name, price, ingredients, media }: { name: string, price: string, ingredients: string, media: string }) {
    return (
...
function OverviewApp({ id, name, category, date, onApprove, onReject }: { id: string | number, name: string, category: string, date: string, onApprove: (id: string | number) => void, onReject: (id: string | number) => void }) {
    return (
