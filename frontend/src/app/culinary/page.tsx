...
export default function ChefDashboard() {
    ...
    const handleScan = async (decodedText: string) => {
        setIsScanning(true);
        try {
            // Using the existing verify-member API endpoint if available or just use memberId
            // The existing VerifyMemberController expects /{qrCode}
            const data = await apiRequest(`/verify-member/${decodedText}`);
        ...
        <div className="min-h-screen bg-brand-ivory flex">
            ...
            <aside className="w-20 md:w-64 bg-brand-brown text-white hidden sm:flex flex-col items-center py-8">
                ...
                <nav className="flex-grow flex flex-col gap-6 w-full px-4">
                    <SidebarLink icon={<ChefHat className="w-5 h-5" />} label="Kitchen Desk" active />
                    <SidebarLink icon={<ClipboardList className="w-5 h-5" />} label="Menus" />
                    <SidebarLink icon={<Clock className="w-5 h-5" />} label="Availability" />
                    <button
                    ...
                    >
                        <div className="max-w-md w-full text-center text-white">
                            <div className="relative w-64 h-64 mx-auto mb-10 border-2 border-brand-gold/30 rounded-[3rem] p-4 group">
                                ...
                                <div className="w-full h-full bg-slate-900 rounded-[2.5rem] flex items-center justify-center relative overflow-hidden">
                                    <Scan className="w-12 h-12 text-brand-gold opacity-50" />
                                    <div className="absolute inset-x-0 h-0.5 bg-brand-gold shadow-[0_0_15px_rgba(196,160,82,0.8)] animate-scan" />
                                </div>
                            </div>
...
function SidebarLink({ icon, label, active = false }: { icon: any, label: string, active?: boolean }) {
    return (
        <button className={`w-full flex items-center gap-4 py-3 px-3 rounded-2xl transition-all ${active ? 'bg-brand-gold/20 text-brand-gold shadow-lg shadow-black/10' : 'text-white/40 hover:bg-white/5 hover:text-white'
            }`}>
...
function OrderCard({ order, confirmed = false, onUpdate }: any) {
