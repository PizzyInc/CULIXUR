...
export default function Dashboard() {
    ...
    const handleDowngrade = async () => {
        if (!confirm("Are you sure you want to cancel your Elite membership? You will lose premium privileges immediately.")) return;
        try {
            await memberApi.cancelSubscription();
            showToast("Subscription cancelled. Welcome back to Culixur Member status.");
            window.location.reload();
        } catch (err) {
            showToast("Cancellation failed. Please contact support.", "error");
        }
    };
    ...
    useEffect(() => {
        const fetchDashboard = async () => {
            try {
                ...
                setUserData({ ...data.user, monthlyOrderCount: data.monthlyOrderCount });
                                ...
                                {upgradeClientSecret ? (
                                    <div className="mt-8">
                                        <Elements
                                            options={{
                                                ...
                                                appearance: { theme: 'stripe', variables: { colorPrimary: '#D4AF37' } }
                                            }}
                                        ...
                                        >
                                            <CheckoutForm
                                                onSuccess={() => {
                                                    ...
                                                    alert("Welcome to Elite. Transitioning privileges...");
                                                    window.location.reload();
                                                }}
                                                buttonLabel="Pay $3,500 & Elevate"
                                            />
                                        </Elements>
                                    </div>
                                ) : (
                                    <button
                                        onClick={async () => {
                                            setIsUpgrading(true);
                                            try {
                                                const res = await memberApi.createSubscription();
                                                if (res.clientSecret) {
                                        ...
                                        className="w-full bg-brand-brown text-white py-5 rounded-full font-bold uppercase tracking-[0.2em] text-[10px] hover:bg-black transition-all shadow-xl shadow-brand-brown/20 disabled:opacity-50"
                                    >
                                        {isUpgrading ? 'Securing Channel...' : 'Initiate Settlement'}
                                    </button>
                                )}
                            </div>
                        </motion.div>
                    ...
                    <div className="flex justify-between h-20">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <button
                                ...
                                className="flex items-center gap-2 px-4 py-2 rounded-full bg-brand-brown/5 hover:bg-brand-brown hover:text-white text-brand-brown transition-all group"
                                title="Logout"
                            >
                                <LogOut className="h-4 w-4" />
                                <span className="text-xs font-bold uppercase tracking-widest hidden lg:inline">Logout</span>
                            </button>
                        </div>
                    </div>
                </div>
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                            ...
                            <div className="relative z-10 flex flex-col h-full justify-between">
                                <div className="flex items-start justify-between mb-8">
                                    <div className="w-16 h-16 rounded-2xl bg-white/10 border border-white/20 p-1 flex-shrink-0 relative group">
                                        ...
                                        <label htmlFor="profile-upload" className="absolute inset-0 bg-black/40 rounded-xl opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity text-[8px] font-bold uppercase cursor-pointer">Update</label>
                                        <input
                                            id="profile-upload"
                                            type="file"
                                            accept="image/*"
                                            ...
                                            onChange={async (e) => {
                                                ...
                                                if (file) {
                                                    ...
                                                    try {
                                                        showToast("Synchronizing profile...");
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        {isElite ? (
                                            <button onClick={handleDowngrade} className="text-[9px] font-bold text-white/40 hover:text-white uppercase underline underline-offset-4 tracking-widest transition-colors">
                                                Downgrade to Culixur Member
                                            </button>
                                        ) : (
                                            <button onClick={() => setShowUpgradeModal(true)} className="text-[9px] font-bold text-white/60 hover:text-white uppercase underline underline-offset-4 tracking-widest transition-colors">
                                                Upgrade to Elite
                                            </button>
                                        )}
                                    </div>
                                ...
                                {isElite ? (
                                    <div className="space-y-4">
                                        <div className="flex items-baseline gap-2">
                                            <span className="font-serif text-5xl font-bold">{userData?.monthlyOrderCount || 0}</span>
                                            <span className="text-white/40 font-bold">/ 15</span>
                                        </div>
                                        <button
                                            onClick={handleDowngrade}
                                            className="px-4 py-2 border border-white/20 rounded-full text-[9px] font-bold uppercase tracking-widest text-white/60 hover:bg-white hover:text-black transition-all"
                                        >
                                            Downgrade to Culixur Member
                                        </button>
                                    </div>
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
                        ...
                        <ActionCard
                            ...
                            label="Refer an Elite"
                            href="/refer-elite"
                            onClick={!isElite ? () => setShowUpgradeModal(true) : undefined}
                        />
...
function ActionCard({ icon, label, href, disabled, onClick }: { icon: any, label: string, href: string, disabled?: boolean, onClick?: (e: React.MouseEvent) => void }) {
    if (disabled) {
        return (
            <div className="bg-white rounded-2xl p-6 text-center opacity-50 cursor-not-allowed border border-brand-brown/5 h-full flex flex-col items-center justify-center">
    ...
    const classes = "bg-white rounded-2xl p-6 text-center border border-brand-brown/5 hover:border-brand-gold hover:shadow-lg hover:shadow-brand-gold/10 group transition-all duration-300 flex flex-col items-center justify-center h-full w-full";
...
// Reuse CheckoutForm logic or import if shared (defined here for dashboard context)
import { loadStripe } from '@stripe/stripe-js';
import { Elements, PaymentElement, useStripe, useElements } from '@stripe/react-stripe-js';
const stripePromise = loadStripe(process.env.NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY || 'pk_test_placeholder');

function CheckoutForm({ onSuccess, buttonLabel = "Secure Payment" }: { onSuccess: (id: string) => void, buttonLabel?: string }) {
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6">
            <PaymentElement />
            {message && <div className="text-red-500 text-[10px] font-bold uppercase tracking-widest">{message}</div>}
            <button
                disabled={isLoading || !stripe || !elements}
                className="w-full bg-brand-gold text-white py-4 rounded-xl text-[10px] font-bold uppercase tracking-[0.2em] hover:bg-black transition-all disabled:opacity-50"
            >
                {isLoading ? "Synchronizing..." : buttonLabel}
            </button>
        </form>
    );
}
